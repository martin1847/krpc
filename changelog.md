...
* **HTTP entry no longer blocks the netty NIO eventLoop (O6).** `handler.handle()` (possibly blocking on DB/downstream) ran on the bounded netty worker eventLoop — a slow handler starved every other connection it served. `handle()` is now dispatched to a per-request **virtual thread**; the response write is scheduled back on the channel's eventLoop (netty model: writes belong to the eventLoop). Backpressure: `setAutoRead(false)` before handoff bounds in-flight blocking work to ≤1 per connection, re-armed after the write. **Caveat:** responses to requests pipelined within a single TCP segment can reorder (documented, rare on the agent/MCP surface, not addressed). No `voidPromise`/AUD-omp-50 entanglement. (O6 + AUD-omp-09; `AbstractHttpHandler.java`. SPEC §12.4.)
