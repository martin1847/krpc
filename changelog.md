...
* **Empty/null JWKS keys fail-closed (O-sec-47).** A `{"keys":[]}` / null-keys document is treated as a failed fetch (no `NullPointerException`, last-known-good keyset retained; a fresh verifier stays not-ready → `UNAVAILABLE`).
